...
public class GlobelException {
    ...
    public ApiResponse ioExceptionadvice(IOException ex) {
        return ApiResponse.builder().statusCode("604").errors(Map.of("message", "Failed to Save File")).build();
    }

    @ExceptionHandler(IllegalStateException.class)
    public ApiResponse illegalStateExceptionadvice(IllegalStateException ex) {
        return ApiResponse.builder().statusCode("604").errors(Map.of("message", "Path Mismatch")).build();
    }

    @ExceptionHandler(BadCredentialsException.class)
    public ApiResponse badCredentails(BadCredentialsException ex) {
        return ApiResponse.builder().statusCode("604").errors(Map.of("message", ex.getMessage())).build();
    }
